...
public class AuthController(IUserService userService, ILogger<AuthController> logger) : ControllerBase
{
    ...
    [SwaggerResponse(200, "User successfully registered", typeof(SuccessResponse<UserJsonResponse>))]
    ...
    {
        _logger.LogInformation("AuthController try REGISTER {Email}", request.Email);
    ...
    {
        _logger.LogInformation("AuthController try LOGIN {RequestLogin}", request.Login);
